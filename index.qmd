...
number-depth: 2 #especifica a quantidade de subníveis a serem numerados abaixo das seções de capítulos
...
execute: 
  ...
  error: true #if false, do not include errors in the output. (note that this implies that errors executing code will not halt processing of the document). #  include: ?? 
...
## Integrar Quarto, RStudio e Github
...
* Criar uma chave PAT, também chamada de token, por um desses meios: 
  ...
  + no console do R/RStudio, digitar:
...
## Associar projeto Quarto ao repositório Github
...
# Criando gráfico com estimativas intervalares

Para este exercício, retomaremos a base da lista anterior, em que foi elaborado um gráfico com informação de proporção da amostra. 

Iremos, contudo, aplicar um dos métodos para estimadores de proporção recomendados por Nascimento [-@nascimento2021, cap.5].
...
## Estimador de proporção (intervalo de confiança)
...
# Os intervalos de confiança em formato tibble
...
# Apresentar uma saída gráfica em ggplot 
...
# Renderização da apresentação final
